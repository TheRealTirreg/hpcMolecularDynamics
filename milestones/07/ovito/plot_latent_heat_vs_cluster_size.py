...
for i, df in enumerate(dfs):
    ...
    b, a = np.polyfit(df[headers[0]].array[:len(df)//3], df[headers[1]].array[:len(df)//3], deg=1)
    ...
    for j, ZIPPEDY in enumerate(zip(t, df[headers[1]].array, (a2 - (a2-a)/2) + (b2 - (b2-b)/2) * xseq)):
        ...
        if np.abs(new_dif) < 1e-12:
            ...
            intersections.append((df[headers[0]].iloc[j], -(ydata * prev_yline - yline * prev_ydata) / denom))
